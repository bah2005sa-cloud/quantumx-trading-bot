...
# ==========================================
# 1. إعدادات تليجرام وحساب المسابقة (100K)
# ==========================================
TELEGRAM_TOKEN = "8888568882:AAGj6udKvmFkg9wldpTgPgc7nxRhRenMDQ0"
TELEGRAM_CHAT_ID = "7691199088"
...
# ⚙️ تخصيص استراتيجيات كل زوج المخصصة للمسابقة (Yahoo Finance Tickers)
ASSETS_CONFIG = {
    "XAUUSD": {
        "ticker": "GC=F", "period": "5d", "interval": "15m", "higher_interval": "1h",
        "tp1_mult": 1.5, "tp2_mult": 3.5, "min_prob": 0.78, "atr_sl_mult": 1.6, "min_adx": 26,
        "contract_size": 100
    },
    "EURUSD": {
        "ticker": "EURUSD=X", "period": "5d", "interval": "15m", "higher_interval": "1h",
        "tp1_mult": 1.5, "tp2_mult": 3.0, "min_prob": 0.75, "atr_sl_mult": 1.2, "min_adx": 24,
    ...
    "US30": {
        ...
        "tp1_mult": 1.5, "tp2_mult": 4.0, "min_prob": 0.82, "atr_sl_mult": 1.8, "min_adx": 26,
        "start_hour": 15, "start_min": 30, "end_hour": 19, "end_min": 0,
        "contract_size": 1
    },
    "GER30": {
        "ticker": "^GDAXI", "period": "5d", "interval": "15m", "higher_interval": "1h",
        "tp1_mult": 1.5, "tp2_mult": 3.5, "min_prob": 0.82, "atr_sl_mult": 1.6, "min_adx": 26,
...
def calculate_dynamic_lot(symbol, sl_dist_price, confidence_prob):
    ...
    risk_pct = 0.02 if confidence_prob >= 0.85 else 0.01
...
# ==========================================
def main():
    print("🏆 Tournament Final Champion Bot (100K Cloud Configured)...")
    send_telegram(
        "🏆 *المحرك الذهبي النهائي للبطولة (حساب 100K سحابي)*\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "🛡️ *التزام كامل بحدود اللوت:* تقييد اللوت بحد أقصى (3 لوت للذهب والمؤشرات / 5 لوت للعملات).\n"
        "⚡ *الخوارزمية:* XGBoost + فلتر سيولة المؤسسات RVOL + توافق فريم H1 + ADX.\n"
        "🎯 *الهدف:* تتبع الأزواج الخمسة بدقة عالية وتحقيق أهداف المسابقة بأمان تام!"
    )
            ...
            for symbol, config in ASSETS_CONFIG.items():
                ...
                if scanned_candles[symbol] != current_candle_time:
                    ...
                    if signal:
                        ...
                        risk_flag = "🔥 فرصة ذهبية (ثقة +85%)" if confidence >= 85 else "🛡️ صفقة قياسية"
